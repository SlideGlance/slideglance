...
A conceptual guide to building layouts with the builder's Flexbox-style layout system, powered by [yoga-layout](https://yogalayout.dev/).

## Choosing a Layout Approach

The builder has four coordinate systems. Picking the right one prevents layout pain.

| System                                                    | When to use                                                                                                   | Example                                            |
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | -------------------------------------------------- |
| **`<VStack>` / `<HStack>`** (Flexbox flow)                | Default for any layout that flows in one axis. Most slide content.                                            | Title + body + footer column                       |
...
Within a single `<Layer>`, prefer `x`/`y` over `position="absolute"` — they both place children absolutely, but `x`/`y` is the canonical form for `<Layer>`. `position="absolute"` is intended for one-off overlays inside flow containers.

> **Note**: PPTX screen readers iterate shapes in **document source order**, not visual order. When using `<Layer>` for complex diagrams, place decorative background shapes first and informational content last so the reading order matches the visual flow. (Future versions may add an `isDecorative` attribute to skip purely visual elements.)

## Layout Containers

The builder provides two Flexbox container types for composing layouts (plus `Layer` for absolute positioning, covered above):
