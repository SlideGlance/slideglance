...
- A measurement-only consumer needs none of that — measurement only
  depends on `slideglance-font`.
- Splitting the wasm boundary keeps that consumer's bundle ~10×
  smaller and lets the two ship on independent release cadences
  without forcing every measurement bump to ride with a parser bump.

Packaged into the `@slideglance/measure` npm package across the same
three target flavors as `slideglance-wasm`.
